...
class Detection
{
...

  {
    ...
    cvtColor(image, image,
             COLOR_BGR2GRAY); // Blur the image for better edge detection
    ...
    {
      Vec4i l = lines[i];
      line(finalImage, Point(l[0], l[1]), Point(l[2], l[3]),
  ...
  bool draw(Mat image, Point2f frontrobotCenter, Point2f backrobotCenter)
  {
    ...
    Rect roiRect(frontrobotCenter.x - roiWidth / 2,
                 frontrobotCenter.y - roiHeight / 2, roiWidth, roiHeight);
    ...
    for (size_t i = 0; i < lines.size(); i++)
    {
      ...
      if (diff < 1)
        if (CalculateDistance(frontrobotCenter, p1) >
            CalculateDistance(backrobotCenter, p1))
        {
    ...
    // rectangle(image, roiRect, Scalar(0, 255, 0), 2); // Draw ROI rectangle on
    // original image
  ...
  bool draw_angle(Mat image, Point2f frontrobotCenter, Point2f backrobotCenter)
  {
    Point2f directionVector =
        computeDirectionVector(backrobotCenter, frontrobotCenter);

    // Normalize the direction vector
    float magnitude = sqrt(directionVector.x * directionVector.x +
    ...
    if (frontrobotCenter.y == backrobotCenter.y &&
        backrobotCenter.x > frontrobotCenter.x)
    {
      topLeft.x = frontrobotCenter.x - roiWidth;
      topLeft.y = frontrobotCenter.y - roiHeight / 2;
    }
    else if (frontrobotCenter.y == backrobotCenter.y &&
             backrobotCenter.x < frontrobotCenter.x)
    {
      topLeft.y = frontrobotCenter.y - roiHeight / 2;
    }
    else if (frontrobotCenter.x == backrobotCenter.x &&
             backrobotCenter.y > frontrobotCenter.y)
    {
    ...
    // Point2f topLeft = roiCenter - directionVector * roiWidth * 0.5 -
    // perpendicularVector * roiHeight * 0.5;
    if (topLeft.x < 0)
      topLeft.x = 0;
    if (topLeft.y < 0)
      topLeft.y = 0;
...
int main(int, char **)
{
  ...
  std::cout << "the robot front" << frontrobotCenter << "the back"
            << backrobotCenter << std::endl;
  ////////////////////////////////////robot front and back
  /// detected/////////////////////
  ...
  // // {
  // //     Vec4i l = lines[i];
  // //     line(linedimage, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(125,
  // 125, 125), 2, LINE_AA);
  // // }
  // for (size_t i = 0; i < lines.size(); i++)
  // {
  ...
  //         if (abs(distanceXS) > 30 || abs(distanceYS) > 30 || abs(distanceXE)
  //         > 30)
  //         {
  //             continue;
  //         }
  //         // Check if the lines are approximately parallel
  //         double angle_diff = abs(atan2(line1[1] - line1[3], line1[0] -
  //         line1[2]) - atan2(line2[1] - line2[3], line2[0] - line2[2])); if
  //         (angle_diff < CV_PI / 5 || angle_diff > CV_PI - CV_PI / 5) { //
  //         Approximately parallel within 10 degrees
  //             Point midpoint1 = calculateMidpoint(Point(line1[0], line1[1]),
  //             Point(line2[0], line2[1])); Point midpoint2 =
  //             calculateMidpoint(Point(line1[2], line1[3]), Point(line2[2],
  //             line2[3])); line(linedimage, midpoint1, midpoint2, Scalar(255,
  //             255, 255), 1); // Draw midpoint
  //         }
  //     }
  // }
